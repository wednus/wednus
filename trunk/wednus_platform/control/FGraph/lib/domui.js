...
//    return( NODE_RADIUS );
  ...
  drawNode: function( node ) {
    try {
      this.getNode(node.id).style.left = (this['frame_left'] + node['position']['x']) +'px';
      this.getNode(node.id).style.top = (this['frame_top'] + node['position']['y']) +'px';
    } catch( e ) {
    }
  },
  ...
  drawFrame: function( frame_width, frame_height ) {
    ...
    // FGraph control - don't change the size of the container
    //this.frame.style.width=parseInt(frame_width) +'px';
    //this.frame.style.height=parseInt(frame_height) +'px';
  },

  // draw the origin
  drawOrigin: function( node ) {
    // FGraph control - WPS automatically does this
    return;
    this.origin.style.left = (this['frame_left'] + node['position']['x']) +'px';
    this.origin.style.top = (this['frame_top'] + node['position']['y']) +'px';
  },
  ...
  addNode: function( node, text ) {
	...
	if ( text ) {
      ...
      domNode.style.MozBorderRadius = (radius * 4) +'px';
    ...
    domNode.style.left = parseInt(node['position']['x']) +'px';
    domNode.style.top = parseInt(node['position']['y']) +'px';
...
// fix prob; http://www.kylescholz.com/blog/2006/06/using_force_directed_graphs.html
textNodeTmpl.style.width = "16px";
textNodeTmpl.style.height = "16px";
textNodeTmpl.style.textAlign = "left";
textNodeTmpl.style.cursor = 'default';
...
pixTmpl.style.width = 2 +'px';
pixTmpl.style.height = 2 +'px';
