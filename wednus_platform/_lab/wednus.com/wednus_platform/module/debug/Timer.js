...
W.Timer = function(){var self = this;
	...
	this.pausedTime = 0;
	this.start = function(){
		W.mesg += 'Test started on '+ (new Date) +'<ul>';
	...
	this.finish = function(){
		...
		// set target message panel
        var target = document.getElementById('_wednus_platform_');
        if(target == null){
            // print queued message
            document.open();
        	document.write(W.mesg);
            document.close();
        }else{
		    //W.mesg += '<a href = "javascript:(function(){var a = document.getElementById(\'_wednus_wps_\'); a.style.display=\'hidden\';})();">Close </a>&nbsp;';
            target.style.display = 'block';
            target.innerHTML = W.mesg;
        }
